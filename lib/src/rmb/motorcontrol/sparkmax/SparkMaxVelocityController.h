...
struct MotorConfig {
  int id;
  rev::spark::SparkMax::MotorType motorType =
...
public:
  ...
  rev::spark::SparkMax &getMotor();

  rev::spark::SparkClosedLoopController &getPIDCOntroller();

  std::unique_ptr<rev::spark::ClosedLoopConfig::FeedbackSensor> &getFeedbackSensor();
...
private:
  mutable rev::spark::SparkMax sparkMax;
  std::vector<std::unique_ptr<rev::spark::SparkMax>> followers;
  rev::spark::SparkMax::ControlType controlType;

  units::radians_per_second_t targetVelocity = 0.0_rpm;
  units::radians_per_second_t tolerance;

  std::unique_ptr<rev::spark::ClosedLoopConfig::FeedbackSensor> encoder;
